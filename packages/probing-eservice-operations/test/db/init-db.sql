...
-- Note: This script provides a temporary approach and should be replaced with a more robust solution in the future to use original DDL.
...
ALTER TABLE
    probing.eservices
ADD
    CONSTRAINT UQ_eservices_eservice_id_version_id UNIQUE (eservice_id, version_id);
...
ALTER TABLE
    probing.eservice_probing_responses
ADD
    CONSTRAINT FK_ESERVICE_ESERVICE_PROBING_RESPONSE FOREIGN KEY (eservices_record_id) REFERENCES probing.eservices(id);
...
ALTER TABLE
    probing.eservice_probing_requests
ADD
    CONSTRAINT FK_ESERVICE_ESERVICE_PROBING_REQUESTS FOREIGN KEY (eservices_record_id) REFERENCES probing.eservices(id);
...
CREATE ROLE "probinguser" WITH NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT LOGIN NOREPLICATION NOBYPASSRLS CONNECTION
LIMIT
    -1 PASSWORD 'mypassword';

GRANT CREATE,
USAGE ON SCHEMA probing TO "probinguser";
...
,
UPDATE
    ON TABLE probing.eservice_probing_requests TO "probinguser";
...
,
UPDATE
    ON TABLE probing.eservice_probing_responses TO "probinguser";
...
,
UPDATE
    ON TABLE probing.eservices TO "probinguser";

GRANT
SELECT
    ON TABLE probing.eservice_view TO "probinguser";

GRANT
SELECT
,
    USAGE ON SEQUENCE probing.eservice_sequence TO "probinguser";